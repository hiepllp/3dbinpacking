...
#define MAXBOXES      1000     /* Max number of boxes plus one */
...
void printboxes(int n, int W, int H, int D, int *w, int *h, int *d, 
                int *x, int *y, int *z, int *bno, char *file)
{
  int i;
  FILE *corr;
 
 
  if( (corr = fopen (file,"a")) == NULL )
  {
	  printf("\nImposible abrir el archivo %s",file);
  }  
														 //fprintf(corr,"TX;TY;TZ; RX;RY;RZ; W;H;D;N;BNO;SC;ZI;TXT\n");
  for (i = 0; i < n; i++) {		
	    fprintf(corr,"%2d;%2d;%2d;0;0;0;%2d;%2d;%2d;%2d;%2d;1;1;BOX\n",x[i],y[i],z[i],w[i],h[i],d[i],i,bno[i]);		
  }
...
int main(int argc, char *argv[])
{
  ...
  if (argc == 7) {  
    ...
    bdim        = atoi(argv[3]);
    type        = atoi(argv[4]);
    nodelimit   = atoi(argv[5]);
    iterlimit   = atoi(argv[6]);
    timelimit   = atoi(argv[7]);
    packingtype = atoi(argv[8]);
    strcpy(file2, argv[9]);    
	TESTS = RANDOMTESTS;	
  }
  else {
  ...
  if( (corr = fopen (file2,"w")) == NULL )  { printf("\nImposible abrir el archivo %s",file); }  
  fprintf(corr,"TX;TY;TZ;RX;RY;RZ;W;H;D;N;BNO;SC;ZI;TXT\n");
  bd = D/2-100;  //bd = 0;
  fprintf(corr,"0;0;0;0;0;0;%2d;%2d;%2d;500;0;0;0;BIN\n",W,H,D);
  fclose(corr);
  ...
  for (v = 1; v <= TESTS; v++) {
    ...
    if (lb == ub) solved++;
    //if (type == 0) 
	printboxes(n, W, H, D, w, h, d, x, y, z, bno,file2);
  }
  
  

  //if( ( corr = fopen ("../../3D.csv","a")) == NULL )  { printf("\nImposible abrir el archivo %s",file); }    
  //fclose(corr);
  
  printf("<a target='_blank' href='./contenedor.php'><h2>View All Containers</h2></a>" ,W,H,D,argv[1]);  
  /*printf("n           = %d\n", n);
  printf("bdim        = %d\n", bdim);
  printf("type        = %d\n", type);
  printf("nodelimit   = %d\n", nodelimit);
