...
#define MAXBOXES     1000  /* max number of boxes (plus one box) */
...
boolean check_iterlimit(long iterations, long max)
{
  if (max == 0) return;
  if (iterations >= max) { 
...
void recpack(allinfo *a, int i, int j, int n, box *f, int rel)
{
  ...
  if (a->subiterat == IUNIT) { 
	...
	if ( check_iterlimit(a->iterat, a->iterlimit)){
		f->bno=f->bno+1;
	}
               ...
               int packingtype)
{
  ...
  //if ((packingtype != ROBOT) &&  (packingtype != GENERAL)) error("bad packtype");
